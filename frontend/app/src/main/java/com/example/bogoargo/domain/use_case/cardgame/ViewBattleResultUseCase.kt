...
class ViewBattleResultUseCase @Inject constructor(
    private val cardGameRepository: ICardGameRepository
) {
    suspend operator fun invoke(matchId: Long): DataResult<BattleResult> {
        return try {
            cardGameRepository.viewBattleResult(matchId)
        } catch (e: Exception) {
            DataResult.Error(DataException.UnknownError(e.message ?: "View battle result failed"))
        }
    }
}