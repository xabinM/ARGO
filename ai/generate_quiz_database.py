...
from data.education_curriculum_integration import EducationCurriculumIntegrator
...
def generate_quiz_database():
        ...
        for grade in suitable_grades:
            ...
            for quiz_num in range(1, 4):
                ...
                try:
                    ...
                    if curriculum_integrator:
                        try:
                            educational_metadata = (
                                curriculum_integrator.get_educational_metadata(
                    ...
                    logger.error(
                        f"퀴즈 생성 실패 - {location}>{spot_name} (학년:{grade}, 번호:{quiz_num}): {e}"
                    )
    ...
    generation_stats["processing_time_seconds"] = int(
            ...
            json.dump(
                {
                    "metadata": {
                        ...
                        "grade_distribution": {
                            str(grade): len(
                                [q for q in quiz_database if q["grade"] == grade]
                            )
...
def log_generation_results(quiz_database: List[Dict]):
    ...
    # 성공률 계산
    success_rate = (
        (generation_stats["total_generated"] / generation_stats["total_requested"])
        * 100
        if generation_stats["total_requested"] > 0
        else 0
    )
...
def main():
    ...
    logger.info(
        f"⏱️ 예상 소요시간: {estimated_quizzes//60 + 1}분 (API 제한에 따라 변동)"
    )
