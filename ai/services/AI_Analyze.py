...
class AI_Analyze:  # 이미지, YOLO 추론 결과, 포즈 추론 결과
    ...
    def filter_poses_by_visibility(self, poses_info):
        ...
        for person_keypoints in poses_info:
            ...
            filtered_poses.append(
                [
                    avg_face_x,
                    face_width,  # 0~1   : 얼굴 중심 x좌표, 얼굴 폭
                    face_max_y,
                    face_min_y,  # 2~3   : 얼굴 y 최댓값, 최솟값
                    left_sh_x,
                    left_sh_y,  # 4~5   : 왼쪽 어깨 x, y
                    right_sh_x,
                    right_sh_y,  # 6~7   : 오른쪽 어깨 x, y
                    shoulder_max_y,  # 8     : 어깨 y 최댓값
                    left_elbow_x,
                    left_elbow_y,  # 9~10  : 왼쪽 팔꿈치 x, y
                    right_elbow_x,
                    right_elbow_y,  # 11~12 : 오른쪽 팔꿈치 x, y
                    elbow_max_y,  # 13    : 팔꿈치 y 최댓값
                    left_wrist_x,
                    left_wrist_y,  # 14~15 : 왼쪽 손목 x, y
                    right_wrist_x,
                    right_wrist_y,  # 16~17 : 오른쪽 손목 x, y
                    wrist_max_y,  # 18    : 손목 y 최댓값
                    left_hip_x,
                    left_hip_y,  # 19~20 : 왼쪽 엉덩이 x, y
                    right_hip_x,
                    right_hip_y,  # 21~22 : 오른쪽 엉덩이 x, y
                    hip_min_y,  # 23    : 엉덩이 y 최솟값
                    left_knee_x,
                    left_knee_y,  # 24~25 : 왼쪽 무릎 x, y
                    right_knee_x,
                    right_knee_y,  # 26~27 : 오른쪽 무릎 x, y
                    knee_min_y,  # 28    : 무릎 y 최솟값
                    left_ankle_x,
                    left_ankle_y,  # 29~30 : 왼쪽 발목 x, y
                    right_ankle_x,
                    right_ankle_y,  # 31~32 : 오른쪽 발목 x, y
    ...
    def print_keypoints(self, pose_select, people_count):
        ...
        if not detected_count:
            ...
            return (
                False,
                f"인식된 사람이 {required_count - detected_count}명 부족합니다",
            )
    ...
    def check_sitting_pose(self, person_idx=0):
        ...
        if knee_min_y is not None:
            knee_hip_diff = max(hip_min_y, knee_min_y) / max(
                min(hip_min_y, knee_min_y), 1e-5
            )
        else:
            knee_hip_diff = 20  # 비교 불가 시 큰 값

        if ankle_min_y is not None:
            ankle_hip_diff = max(hip_min_y, ankle_min_y) / max(
                min(hip_min_y, ankle_min_y), 1e-5
            )
        else:
    ...
    def check_handsup_pose(self, person_idx=0, hand="all"):
        ...
        left = [
            self.poses_info[person_idx][5],
            self.poses_info[person_idx][10],
            self.poses_info[person_idx][15],
        ]
        right = [
            self.poses_info[person_idx][7],
            self.poses_info[person_idx][12],
            self.poses_info[person_idx][17],
        ]
    ...
    def check_akimbo_pose(self, person_idx=0):
        ...
        def arm_folded(v_list):
            x_shoulder, y_shoulder, x_elbow, y_elbow, x_wrist, y_wrist, x_hip, y_hip = (
                v_list
            )
    ...
    def check_heart_pose(self, person_idx=0):
        ...
        if None not in [left_shoulder_x, right_shoulder_x, left_elbow_x, right_elbow_x]:
            if min(left_shoulder_x, right_shoulder_x) < min(
                left_elbow_x, right_elbow_x
            ) or max(left_shoulder_x, right_shoulder_x) > max(
                left_elbow_x, right_elbow_x
            ):
