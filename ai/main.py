...
# === 헬퍼 함수들 ===
def convert_to_quiz_problem_simple(quiz_data: Union[Dict, Any], grade: int, fallback_name: str) -> QuizProblemSimple:
    ...
    if hasattr(quiz_data, 'choices'):
        ...
        return QuizProblemSimple(
            ...
            explanation=quiz_data.get("explanation", f"{fallback_name}에 대한 설명입니다."),
            grade=grade,
            spotName=fallback_name
        )
    else:
...
async def generate_problem(request: ProblemGenerateRequest):
    ...
    try:
        ...
        if quiz_service and system_status["quiz_service_ready"]:
            # 퀴즈 서비스 사용
            try:
                problems = await quiz_service.generate_problems(
                    spot_name=request.spotName,
                    grade=request.grade,
                    problem_count=request.problemCnt
                )
                
                for problem in problems:
                    quiz_simple = convert_to_quiz_problem_simple(problem, request.grade, request.spotName)
...
async def pose_predict_full(
    ...
    try:
        ...
        return {
            "success": True,
            "result": f"포즈 분석 성공: {pose_select}",
            "details": {
                "pose_type": pose_select,
