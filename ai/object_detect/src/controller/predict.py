...
@router.post("/predict/")
async def predict(file: UploadFile = File(...), pose_select: str = Form(None)):
    try:
        # 1. 이미지 읽기 및 디코딩
        ...
        if results is None:
            ...
            raise HTTPException(
                status_code=404, detail="사람 감지 실패 / 포즈 탐색 실패"
            )

        # 3. 후처리
        processor = AI_Analyze(image, results, poses_info)
        json_result = processor.print_keypoints(pose_select=pose_select)
